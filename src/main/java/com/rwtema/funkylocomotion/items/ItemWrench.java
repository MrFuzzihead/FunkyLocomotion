...
public class ItemWrench extends Item {
    ...
    @Override
    public boolean onItemUseFirst(ItemStack item, EntityPlayer player, World world, int x, int y, int z, int face,
        float hitX, float hitY, float hitZ) {
        if (item.getItemDamage() != metaWrenchHammer) return false;
        ...
        if (block instanceof BlockStickyFrame) {
            ...
            for (int i = -2; i <= 2; i++) {
                for (int j = -2; j <= 2; j++) {
                    ...
                    if (!blockingBlock.isAir(world, dx2, dy2, dz2)) {
                        if (blockingBlock.getCollisionBoundingBoxFromPool(world, dx2, dy2, dz2) != null
                            && block.canCollideCheck(world.getBlockMetadata(dx2, dy2, dz2), false)) {
                            if (block.collisionRayTrace(
                                world,
                                dx2,
                                dy2,
                                dz2,
                    ...
                    if (i == 0 && j == 0) {
                        ...
                        if (otherBlock instanceof BlockStickyFrame
                            && ((((BlockStickyFrame) otherBlock).index + world.getBlockMetadata(dx, dy, dz))
                                & (1 << face)) == open) {
            ...
            for (int i = -2; i <= 2; i++) {
                for (int j = -2; j <= 2; j++) {
                    ...
                    if (testBlock.isAir(world, dx, dy, dz)) continue;
    ...
    private boolean isInaccessible(World world, int face, float hitX, float hitY, float hitZ, Block block, int dx2,
        int dy2, int dz2, Block blockingBlock) {
    ...
    public String getUnlocalizedName(ItemStack itemstack) {
        if (itemstack.getItemDamage() == metaWrenchEye) return "item.funkylocomotion:wrench_eye";
