...
public class ItemBlockFrame extends ItemBlockMetadata {
    ...
    public ItemBlockFrame(Block block) {
        super(block);
        if (!(block instanceof BlockStickyFrame)) throw new IllegalArgumentException("Frame Block/Item mismatch");
    ...
    public String getItemStackDisplayName(ItemStack itemstack) {
        if (index == 0 && itemstack.getItemDamage() == 0) return super.getItemStackDisplayName(itemstack);
        ...
        for (int i = 0; i < 6; i++) if (((meta) & (1 << i)) != 0)
