...
public abstract class TileMovingBase extends TileEntity {
    ...
    protected static AxisAlignedBB[] AxisTags(NBTTagList tagList) {
        ...
        for (int i = 0; i < n; i++) bbs[i] = AxisNBT(tagList.getCompoundTagAt(i));
    ...
    protected static NBTTagList TagsAxis(AxisAlignedBB[] bbs) {
        ...
        for (AxisAlignedBB bb : bbs) tagList.appendTag(NBTAxis(bb));
        return tagList;
    }
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        if (tag.hasKey("Collisions", 10)) collisions = AxisTags(tag.getTagList("Collisions", 10));
    ...
    public void writeToNBT(NBTTagCompound tag) {
        ...
        if (collisions.length > 0) tag.setTag("Collisions", TagsAxis(collisions));
    ...
    public Vec3 getMovVec() {
        ...
        if (dir < 0 || dir >= 6) return Vec3.createVectorHelper(0, 0, 0);

        return Vec3.createVectorHelper(
            Facing.offsetsXForSide[dir] * d,
            Facing.offsetsYForSide[dir] * d,
    ...
    public AxisAlignedBB getCombinedCollisions() {
        ...
        for (AxisAlignedBB collision : collisions) {
            if (bb == null) bb = collision.copy();
            else bb.func_111270_a(collision);
        }
        ...
        if (dir != ForgeDirection.UNKNOWN) {
            ...
            if (other instanceof TileMovingBase) {
                ...
                for (AxisAlignedBB bb1 : bbs1) {
                    if (bb == null) bb = bb1.getOffsetBoundingBox(dir.offsetX, dir.offsetY, dir.offsetZ);
                    else bb.func_111270_a(bb1.getOffsetBoundingBox(dir.offsetX, dir.offsetY, dir.offsetZ));
                }
            }
        }
    ...
    public AxisAlignedBB[] getTransformedColisions() {
        ...
        for (int i = 0; i < collisions.length; i++) {
            tbbs[i] = collisions[i].getOffsetBoundingBox(h * dir.offsetX, h * dir.offsetY, h * dir.offsetZ)
